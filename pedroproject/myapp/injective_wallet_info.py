            ...
            for model in first_fetch['models']:
                try:
                    ...
                    if value_decoded.isdigit():
                        ...
                        if amount_Coin > 0 and inj_address == self.address:
                            cw20_balances_for_token.append({'denom': token['native'], 'amount': amount_Coin})
                except (ValueError, json.JSONDecodeError):
                    continue
            return cw20_balances_for_token

        # Use asyncio.gather to run multiple fetches concurrently
        tasks = [fetch_token_balance(token) for token in cw20_tokens]
        results = await asyncio.gather(*tasks)

        # Flatten the list of results from each token
